...
- [ ] **CSS aufräumen** — nach 10.2 (Button/Input/Card-Extraktion) verbleibende tote/duplizierte Styles projektweit entfernen. Die Haupt-Duplikate (Pill-Input-Styles in ~8 Dateien, Pill-/Icon-Button-Styles in ~15 Dateien) sind mit der `app-input`- und `app-button`-Komponentenmigration bereits verschwunden; verbleibend sind kleinere Reste (z. B. dead CSS wie das ungenutzte `.changepassword` in `forgot-password.component.scss`, bereits entfernt) und ein finaler projektweiter Sweep.
