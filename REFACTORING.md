...
- [x] **Input-Komponente extrahieren** — aus demselben Grund ebenfalls eine Direktive (`[appInput]`) statt Wrapper: `formControlName`/`ngModel` funktionieren über Angulars eingebauten Value Accessor bereits nativ, die Direktive ergänzt nur eine stabile `id` (wiederverwendet eine vorhandene, sonst generiert) sowie `aria-invalid`/`aria-describedby`. Die eigentliche Fehleranzeige ist eine separate `<app-field-error>`-Komponente (Sibling, kein Wrapper), die das mehrfach duplizierte `@if (invalid && (touched||dirty)) { span.error }`-Markup zentralisiert — welche Meldung angezeigt wird, bestimmt weiterhin die jeweilige Komponente (bewusst kein generischer Message-Resolver, siehe 9.3). Alle Reactive-Form- und ngModel-Felder migriert.
  Dateien: `shared/components/input/input.directive.ts`, `field-error.component.ts`
