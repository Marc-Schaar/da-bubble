...
- [x] **Tests implementieren** — vollständige Testabdeckung ergänzt: **1084 Unit-Tests** (Karma/Jasmine, `ng test`) über alle 18 Services/Stores, alle Guards/Routen in `src/core/`, sowie alle 45 Komponenten — 0 Fehlschläge, Coverage 97,15 % Statements / 90,81 % Branches / 97,33 % Funktionen / 97,68 % Zeilen. Firestore-/Auth-nahe Services (`UsersApiService`, `ChannelsApiService`, `MessagesApiService`, `UnreadApiService`, `AuthService`, beide Auth-Guards) laufen gegen die neu eingerichtete **Firebase Emulator Suite** (`firebase.json`, `firestore.rules`, `.firebaserc`) statt gegen SDK-Mocks. Zusätzlich ein **Playwright-E2E-Suite** (`e2e/`, 8 Spezifikationsdateien, Desktop-/Mobile-Viewport-Projects) entlang der Smoke-Test-Checkliste unten, mit Emulator-Seeding (`e2e/lib/seed-emulator.mjs`); im ersten vollständigen Live-Lauf 32/54 Testinstanzen grün (Kernflows Login/Gast-Login/Channel-Nachrichten/Deep-Links live verifiziert), 17 Fehlschläge bei neueren Specs (Such-Dropdown-Interaktionen, einzelne Mobile-Timing-Fälle) sind als offener Nacharbeits-Punkt dokumentiert, nicht stillschweigend als erledigt markiert. Neuer CI-Workflow `.github/workflows/test.yaml` (Unit- dann E2E-Job, PR/Push-getriggert, unabhängig von `deploy.yaml`). Dabei mehrere echte Bugs gefunden und dokumentiert — wichtigster war: `sign-in.component.ts`s `onSubmit()` brach bei ungültigem Formular nicht ab (fehlendes `return`) und rief den Login trotzdem mit leeren Feldern auf. Inzwischen gefixt (`return` nach `markAllAsTouched()` ergänzt), zugehöriger Test aktualisiert.
- [x] **Dokumentation erweitern** — README um Architektur-Abschnitt (Feature-Layering, Firestore-API-Fassade, Signal-State, Guards, Lazy Loading), Hilfsskripte und eine korrigierte Setup-Anleitung ergänzt: Firebase-Konfiguration liegt tatsächlich in gitignoreten `environment.ts`/`environment.prod.ts` (aus `.example.ts`-Vorlagen kopiert, CI generiert die Prod-Variante aus Secrets) — die alte README-Angabe verwies fälschlich auf `app.config.ts`.
